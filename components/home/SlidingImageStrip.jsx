...
export default function SlidingImageStrip() {
  // Duplicate array for seamless infinite sliding loop
  const duplicatedList = [...STRIP_PROPERTIES, ...STRIP_PROPERTIES];

  return (
    <div className="relative w-full overflow-hidden bg-white py-8 border-b border-slate-100">
      {/* Header with Title and live sliding indicator */}
      <div className="container-box mb-5 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="heading-display text-base sm:text-lg font-bold text-slate-900">
            Live Property Stream • Sliding Left to Right
          </h3>
      ...
      {/* Edge Gradient Mask for smooth fade on borders */}
      ...
      {/* Infinite Left-to-Right Animated Track */}
        ...
        <div className="animate-slide-ltr flex gap-5 py-2">
          {duplicatedList.map((item, idx) => (
            <Link
              key={`${item.id}-${idx}`}
              href={item.link}
              className="group relative flex-shrink-0 w-[270px] sm:w-[320px] overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:border-[#a98440]/40"
            >
